...
**Destructive actions.** Log Out (`settings_screen.dart`) and the Community Feed/Community Post Details moderator actions (remove post, remove comment, ban member — `community_feed_screen.dart`, `community_post_details_screen.dart`) confirm via an `AlertDialog` — Trip Details' Cancel Trip is the one exception, firing immediately with no confirmation, so don't treat that as the precedent for a new destructive action. Give the dialog's confirm button text distinct from the triggering row/button's label (e.g. row "Log Out" vs. dialog "Yes, Log Out", row "Remove post" vs. dialog "Yes, Remove") so `find.text`/Maestro's `tapOn` can't ambiguously match both.
