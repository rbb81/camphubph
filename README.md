...
- [`test/camp_owner_dashboard_screen_test.dart`](test/camp_owner_dashboard_screen_test.dart) — business header renders (distinct from camper Profile — no Followers/Following), seeded reservations render, Confirm/Decline flips a pending reservation's status and updates `sampleReservations`, Add Reservation appends a new card
...
3. In another terminal, run a flow. Every file below has been run against a real Chrome window in this environment (chromedriver 149.x) — **all 20 currently pass**:
   ...
   | `camp_owner_dashboard_test.dart` | Pumps `CampOwnerDashboardScreen` directly (sidesteps login-gating, same as `home_test.dart`); checks the business header/seeded reservations render, Confirm/Decline flips a pending reservation's status, and Add Reservation appends a new one. |
