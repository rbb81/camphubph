...
[`lib/screens/follow_list_screen.dart`](lib/screens/follow_list_screen.dart) (`FollowListScreen`, one screen parameterized by `FollowListType.followers`/`.following`) is reached via Profile's follower/following stat taps. Both lists draw from the existing 5-person [`lib/data/sample_other_users.dart`](lib/data/sample_other_users.dart) directory rather than the display-only follower/following counts on `UserProfile` (which have no backing identities at all) — Following is real (filtered by the existing `FollowStatus.following`), Followers needed a new fixed-seed `followsMe` bool on `FollowableUser` since the reverse relationship wasn't modeled. Each row reuses `OtherUserProfileScreen`'s exact follow/unfollow state machine and taps through to that person's profile. `OtherUserProfileScreen`'s own follower/following stats remain non-interactive — showing someone else's followers/following is a bigger, separate ask.

## Hashtags & Mentions

Post/comment bodies across Home Feed, Post Details, Community Feed, and Community Post Details render through [`lib/widgets/hashtag_mention_text.dart`](lib/widgets/hashtag_mention_text.dart) (`HashtagMentionText`) — the first inline-tappable-text primitive in this codebase, and the first shared widget in `lib/widgets/` beyond `auth_layout.dart`'s auth-screen helpers. `#hashtag` tokens open [`SearchScreen`](lib/screens/search_screen.dart) pre-filled with that term (a new optional `initialQuery` constructor param); `@mention` tokens open that person's [`OtherUserProfileScreen`](lib/screens/other_user_profile_screen.dart) — since there's no username/handle system, a mention matches a display name with spaces stripped (`@MiguelIbarra` → "Miguel Ibarra"), case-insensitively, and self-mentions route to `/profile` instead. An unrecognized handle still opens a profile backed by a synthesized placeholder, matching this app's other name-lookup fallback conventions. Scope is read-time rendering + tap navigation only — Create Post's caption field doesn't autocomplete or highlight hashtags/mentions while typing, and Profile bios/Review snippets aren't touched.
...
- [`test/search_screen_test.dart`](test/search_screen_test.dart) — empty-query prompt; case-insensitive camp/community/person matching and tap-through to their detail screens; Locations and Activities chips navigate into `CampResultsScreen`; the All facet groups results into headed sections with a "See all in X" affordance; no-results empty state; clearing the query resets to the prompt; an `initialQuery` pre-fills the field and shows results immediately
- [`test/hashtag_mention_text_test.dart`](test/hashtag_mention_text_test.dart) — plain text unaffected; a hashtag/mention span fires its callback with the token minus its prefix; trailing punctuation is excluded from the token; mixed plain+tagged text renders in order; `openHashtagSearch` opens Search pre-filled; `openMentionedProfile` self-mentions route to `/profile`, known handles resolve to the real profile, unknown handles still open a synthesized one
- [`test/camp_details_screen_test.dart`](test/camp_details_screen_test.dart) — identity block renders, bookmark toggle, Reviews tab (populated and empty states), writing a review updates the aggregate rating/count, Add to Trip pushes Schedule Trip and shows a confirmation, Message Campsite reuses an existing thread or starts a new empty one, Map tab renders a mini-map with a working "View on Map" button that opens the full Map screen centered on that camp, Photos tab empty state and tapping a tile opens the fullscreen photo lightbox
- [`test/photo_lightbox_screen_test.dart`](test/photo_lightbox_screen_test.dart) — shows a 1-based "N of total" counter starting at the first photo, honors a non-zero `initialIndex`, swiping updates the counter
...
- [`test/community_feed_screen_test.dart`](test/community_feed_screen_test.dart) — pinned posts, Rules/Members tabs, like toggle, compose (gated on membership), request-to-join → approval flow, the comment icon opens Community Post Details and a new comment updates the feed card's count, the "Pending join requests" entry point is moderator-only and shows the pending count, approving a request there updates the Members roster on return, a moderator can remove a post from the feed (with confirm/cancel) and ban a regular member from the Members tab, both gated to moderators only
- [`test/community_post_details_screen_test.dart`](test/community_post_details_screen_test.dart) — full post + existing comments render, like toggle, submitting a comment appends it and bumps the count, an empty comment is ignored, empty state with no comments, popping returns the updated post, a non-moderator sees no remove controls, a moderator can remove a comment or remove the post (which pops `null` instead of the post)
...
- [`test/profile_screen_test.dart`](test/profile_screen_test.dart) — identity block and tab labels render, switching tabs shows matching sample content, tapping a post/photo/review opens its detail view, tapping a saved camp or wishlist item opens Camp Details (falling back to a placeholder Camp if unmatched), tapping a completed trip opens a read-only trip summary whose View Camp button opens Camp Details, Edit Profile and Trip Planner navigation, opening Settings and editing the profile from within it updates Profile on return, the followers/following stats open the Followers/Following list
- [`test/follow_list_screen_test.dart`](test/follow_list_screen_test.dart) — Followers shows only `followsMe` users, Following shows an empty state when no one is followed, tapping a row opens that person's Other User Profile, Follow Back cycles to Requested then auto-approves to Following, tapping Following unfollows immediately and removes them from the list
...
2. Leave that running, and in a second terminal run a flow against it:
   ...
   Home is only reachable after logging in. The flow's default credentials (`dummy@example.com` / `dummy-password`) work as-is against a build without `--dart-define-from-file`, since the dummy auth fallback accepts any credentials. Against a build with real Supabase credentials, override with a pre-existing, **email-confirmed** test account instead: `maestro test -e MAESTRO_TEST_EMAIL=you@example.com -e MAESTRO_TEST_PASSWORD=yourpassword .maestro/home_smoke.yaml`. All of the flows below share this same login requirement and credential-override mechanism. Also taps the `#Daraitan` hashtag in a caption (plain rendered text, no tooltip needed) and checks it opens Search already showing the matching camp.

   ```bash
   maestro test .maestro/profile_smoke.yaml
   maestro test .maestro/edit_profile_flow.yaml
   maestro test .maestro/follow_list_flow.yaml
   ```

   `profile_smoke.yaml` logs in, taps into Profile from the bottom nav, and checks a tab switch; `edit_profile_flow.yaml` additionally opens Edit Profile, changes the name field, saves, and confirms the change is reflected back on Profile. `follow_list_flow.yaml` opens Followers, taps "Follow Back" on the first seeded follower (`index: 0`, since multiple rows share that label), confirms it flips to Requested then Following, then checks the same person now appears under Following too.
...
3. In another terminal, run a flow. Every file below has been run against a real Chrome window in this environment (chromedriver 150.x) — **all 30 currently pass**:
   ...
   | `home_test.dart` | Feed renders; search opens the Search screen; create-post opens Create Post; like toggle; tap-through to Post Details, Camp Details, Discover, Map, Communities; tapping a friend-post author opens Other User Profile; tapping a `#hashtag` in a caption opens Search pre-filled with a matching result. |
   ...
   | `search_test.dart` | Typing a camp/community/person query and tapping the result opens Camp Details/Community Feed/Other User Profile; the Locations and Activities chips navigate into a filtered Camp Results; an `initialQuery` pre-fills the field and shows results immediately. |
   | `camp_details_test.dart`, `write_review_test.dart` | Reviews tab renders; writing a review updates the aggregate rating/count live; review-form validation and pro/con chip add; Message Campsite opens a thread and a sent message appears; the Map tab's "View on Map" button opens the full Map screen centered on that camp; tapping a Photos tab tile (with an injected real photo) opens the fullscreen photo lightbox. |
   ...
   | `other_user_profile_test.dart` | Pumps `OtherUserProfileScreen` directly (same pattern as `home_test.dart`); renders the identity block; requesting to follow flips to Requested then Following after a real (non-simulated) delay; Message opens a thread and a sent message appears; tapping a Photos tab tile (with an injected real photo) opens the fullscreen photo lightbox; Reviews tab renders this user's reviews and taps through to Camp Details. |
   | `photo_lightbox_test.dart` | Pumps `PhotoLightboxScreen` directly; swiping to the next photo updates the "N of total" counter. |
   ...
   | `create_post_test.dart`, `post_details_test.dart` | Caption validation and cancel; like toggle and adding a comment updates the thread; tapping an `@mention` in a comment opens that person's Other User Profile. |
   ...
   | `community_post_details_test.dart` | Liking bumps the like count; submitting a comment appends it to the thread; a moderator removes a comment and then removes the post. |
...
lib/screens/                landing, auth, home, notifications, search, discover/camp results/camp details/write review/photo lightbox, map, schedule trip/trip planner/trip details, create post/post details, communities/community feed/community post details/create community/pending join requests, profile/edit profile/settings/follow list screens
