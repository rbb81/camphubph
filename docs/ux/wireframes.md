...
- **Comments implemented (Phase 3, 2026-07-24):** tapping a post's comment icon (previously `_comingSoon`) opens `lib/screens/community_post_details_screen.dart` — a like toggle, flat (non-threaded) comment list, and composer, mirroring Post Details' exact shape for a `CommunityFeedPost` instead of a `FriendPostItem` (kept as a separate screen since the two post types are unrelated models, per this app's no-shared-tile-widget convention). `CommunityFeedPost` gained a `comments: List<Comment>` field (reusing the existing generic `Comment` model), tracked independently of `commentCount` — same convention as `FriendPostItem`. Not yet built: photo attachments on community posts, moderator tools to edit rules/members. **Hashtags & Mentions (2026-07-24):** post bodies and comments here render the same tappable `#hashtag`/`@mention` tokens as Post Details — see that section's note for details.
- **Moderator join-request review implemented (Phase 3, 2026-07-24):** the Members tab shows a "Pending join requests" row (with a count badge) when the current user moderates that community, opening `lib/screens/pending_join_requests_screen.dart`. Approve adds the requester as a real `CommunityMember` and bumps the community's member count; Decline just removes the request — there's no status/history kept for either (unlike Notifications' follow-request chip), since nothing else in this app needs to look back at resolved requests. New data: `lib/models/community_join_request.dart` (`CommunityJoinRequest`, no status field) + `lib/data/sample_community_join_requests.dart`, and a new seeded private community, "Bicol Volcano Trekkers" (`bicol-volcano-trekkers`), where the current user is pre-seeded as a moderator — added rather than repurposing `palawan-dreamers`, which is the demo for the current user's own *outgoing* "Request to Join" flow and would conflict.
- **Moderator content actions implemented (Phase 3, 2026-07-24):** closes out the MVP's "basic trust & safety" requirement (`docs/features.md`) — remove post, remove comment, ban member. A moderator now sees a trash icon on each post card (Feed tab and Community Post Details) and on each comment in Community Post Details, and a block icon next to non-moderator rows in the Members tab; each action confirms via an `AlertDialog` first (matching Settings' Log Out pattern). Remove-post/remove-comment only mutate the screen's local post/comment list (same as existing like/comment edits — not written back to the canonical `sampleCommunityPosts`); ban-member mutates the shared `sampleCommunityMembers` list directly, matching the pending-join-request approve/decline convention, and decrements the community's member count. `Comment` gained a required `id` field (needed to target a specific comment for removal) — a small blast radius across `sample_feed.dart`/`sample_community_posts.dart`'s seed data and the two post-details screens' comment composers. Not in scope: moderators can't ban a fellow moderator, there's no ban list preventing a re-join, and Home Feed/Post Details (friend posts, not community posts) have no moderation — only community-posted content is covered, per the MVP spec.
...
- **Implemented (Phase 3, 2026-07-08):** `lib/screens/post_details_screen.dart` — full post, like toggle, flat (non-threaded) comment list, comment composer. Not yet built: follow button, share/bookmark/report, moderation state.
- **Hashtags & Mentions implemented (Phase 3, 2026-07-24):** the post body and every comment now render through `lib/widgets/hashtag_mention_text.dart` (`HashtagMentionText`) — `#hashtag`/`@mention` tokens are tappable: a hashtag opens Search pre-filled with that term; a mention opens that person's profile (self-mentions route to the current user's own Profile). Same treatment applies to Home Feed captions and Community Feed/Community Post Details bodies and comments. Since there's no username/handle system, a mention matches a display name with spaces stripped (`@MiguelIbarra` → "Miguel Ibarra"), case-insensitively; an unrecognized handle still opens a profile backed by a synthesized placeholder, same convention as this app's other name-lookup fallbacks. Not built: composer-side autocomplete/highlighting while typing (see Create Post below), a dedicated "posts tagged X" feed (hashtag taps reuse the existing Search screen instead), and this treatment on Profile bios or Review snippets.
...
- **Implemented (Phase 3, 2026-07-08):** `lib/screens/create_post_screen.dart` — required caption, optional location, single photo picker (via `image_picker`), publishes a `FriendPostItem` to the top of Home Feed. Not yet built: multi-image/reorder, @mention/#hashtag support in the composer itself (autocomplete/highlighting while typing), camp/community tagging, post-type quick-select, upload progress (no Storage backend yet — photo stays in-memory). Note: a caption typed with a literal `#tag`/`@name` *does* render as tappable once published, since Hashtags & Mentions (see Post Details above) is display-time, not authoring-time.
...
- **Lightbox implemented (Phase 3, 2026-07-25):** `lib/screens/photo_lightbox_screen.dart` — a full-screen `PageView` of the tapped photo grid (Camp Details' Photos tab, and Other User Profile's Photos tab, which aggregates that user's post + review photos), with a "N of total" counter, swipe between photos, and pinch-to-zoom via `InteractiveViewer`. A generic reusable screen (not duplicated per-screen) since the swipe/index-tracking logic would otherwise be copy-pasted verbatim — same rationale as `lib/widgets/`'s cross-cutting-logic exception in CLAUDE.md, just realized as a pushed screen instead of an embedded widget. **Not yet implemented:** no source-post/review link or uploader credit inside the lightbox, and no dedicated standalone "Photo Gallery" screen reached independently of a camp/profile — it's only reached by tapping into an existing Photos tab. The current user's own Profile "Photos" tab (`sampleProfilePhotos`) is a separate, pre-existing gap: it's caption-only seed data with no real image bytes at all (unlike Camp Details/Other User Profile, which pull real `Uint8List` photos from reviews/posts), so there's nothing for a lightbox to display there yet — tapping it still opens the older text-only `ProfileSimpleDetailScreen`. Maestro coverage intentionally skipped: none of the seeded reviews/posts carry real photo bytes (Uint8List image data can't be embedded as Dart const literals), so a Photos tab is only ever populated by actually picking a photo through Write a Review's `image_picker` flow — and that opens the native OS gallery UI, which (per `create_post_flow.yaml`'s existing precedent) isn't reliably drivable by Maestro. The happy path is covered by widget and chromedriver tests instead, which inject real decodable bytes directly into `sampleReviews`.
