...
- **Implemented, narrower scope (Phase 3, 2026-07-09):** this general user-to-user inbox described above is **not** built. What shipped instead is camper↔campsite messaging specifically: `lib/models/message_thread.dart` (`MessageThread`/`ChatMessage`), sample data in `lib/data/sample_message_threads.dart` (shared mutable list, same pattern as Reservations/Trips — no real backend, no persistent read/unread state). Camper side: a "Message Campsite" button on Camp Details opens `lib/screens/message_thread_screen.dart`, a two-way chat (message bubbles, composer, no timestamps-as-"sent/failed" states). Owner side: the Camp Owner Dashboard's new Messages section lists threads (guest name, camp name, last-message preview, sorted by recency) and opens the same thread screen to reply. Out of scope: unread badges, a general inbox screen, messaging other users (not just campsites), participant-profile navigation, push notifications.
